...
3. **Ship mechanics — investigated and largely closed 2026-08-08** (MJ final copy throughout). Done: the **three-state readout fix** (no moves / measured effect / moves-that-measured-zero each get their own honest line — `turnLoop.js` now records `playerMoves` on the result, additive only, gates untouched); **both READMEs restructured play-path-first** (ZIP → extract → double-click; npm demoted to "For developers"); **launcher hint** now leads with the real likeliest failure (unextracted ZIP) with the build note demoted to a developers line; **repo description** updated; **GitHub Release v1.0.0** cut for a stable link. ZIP artifact verified end-to-end earlier the same day (prebuilt dist ships, full turn plays from the extracted archive, zero console errors). **Pre-first-result ask verified clean** (stubbed-fetch live test: well-formed payload with `this_turn_result: null`, both voices render, no error path). **Still open, MJ-manual: the `file://` double-click re-verification** (post-rules.html/favicon; additions are inert but unverified from a true `file://` origin — DO NOT claim verified) and any second-browser smoke MJ wants.
