...
2. **Sound — ✅ BUILT 2026-08-10.** Tiers 1/2/5 + Poll-Close Drumroll per `game/SOUND_SPEC.md`, all **WebAudio-synthesized** in `game/src/ui/sound.js` — no audio files, no fetch, works unkeyed and from `file://` by construction. Hooks in `main.js` only (the spec's original `turnLoop`/`evaluateEnd` hook points are gate-driven pure functions — audio there would break `gate:all`; spec updated). Drumroll holds the results REVEAL ~1.8s, engine resolves instantly; muted → no delay. Tier 2 = player wins delegates where they campaigned (uses the `playerMoves` field). No-stacking enforced by construction (one cue per batch, highest wins); Tier 5 once per game. Mute toggle in header, session-persisted, kills audio instantly. **No Tier 4 path exists — confirmed by absence**: no cue for Tiers 3/4 is defined anywhere, and the shipped bundle greps clean for it. Browser-verified: reveal-hold + instant muted reveal + full muted season to conclusion, zero console errors. **5a fanfare is the documented placeholder** — Doorkeeper Original Music sting swaps in at `cues.fanfare` post-ship. **SOUND APPROVED — MJ full listen 2026-08-10:** tick, chime, drumroll, and end cue all land; pacing holds across a season; no changes. Tag `v1.1.0` created at `3934724`; **release PUBLISHED 2026-08-10** (notes approved by MJ as written): `https://github.com/Doorkeeper-MJ/election-game/releases/tag/v1.1.0`. Pending: MJ's 10-second double-click of the v1.1.0 ZIP, and the item-4 cold test (see DoD item 4).
3. **Ship mechanics — investigated and largely closed 2026-08-08** (MJ final copy throughout). Done: the **three-state readout fix** (no moves / measured effect / moves-that-measured-zero each get their own honest line — `turnLoop.js` now records `playerMoves` on the result, additive only, gates untouched); **both READMEs restructured play-path-first** (ZIP → extract → double-click; npm demoted to "For developers"); **launcher hint** now leads with the real likeliest failure (unextracted ZIP) with the build note demoted to a developers line; **repo description** updated; **GitHub Release v1.0.0** cut for a stable link. ZIP artifact verified end-to-end earlier the same day (prebuilt dist ships, full turn plays from the extracted archive, zero console errors). **Pre-first-result ask verified clean** (stubbed-fetch live test: well-formed payload with `this_turn_result: null`, both voices render, no error path). **`file://` re-verification — ✅ CLOSED 2026-08-10, verified by MJ on the RELEASE ARTIFACT, not inferred:** downloaded the v1.0.0 release ZIP, extracted to `Downloads\election-game-1.0.0 (1)\election-game-1.0.0\` (path with spaces and parentheses — held), double-clicked root `index.html` from `file://`: launcher redirect fired, Quick Start modal opened, `rules.html` opened from the modal footer, **full race played to completion** (Trump, Cruz second, Carson third, Rubio fourth). Broadcast voices held character — **keyed path** (a key was stored in that browser), which also re-confirms the browser-direct API from a `file://` origin on the shipped artifact. **DoD item 2 is verified on the release artifact.**
...
**Ship close-out — run the v1 Definition of Done as a checklist.** All build items are done. Remaining: (1) **MJ listens to the sound layer once** (synth quality + drumroll pacing are his call; a re-cut release would carry any tweaks); (2) walk DoD items 1–5 and declare, or fix what fails; (3) decide whether v1.0.0 stands as the ship tag or a v1.1.0 release is cut with sound included — **the v1.0.0 release ZIP predates the sound layer**, so shipping sound requires a new tag/release.
...
4. **Slice 4 done to "clear," not "perfect."** Quick Start modal (goal / levers / turn structure / win condition) + static **"?"** tooltips per panel. **Static help only — no Tutorial Mode.** Good enough that a stranger isn't confused; not polished to a standard nobody asked for.
   > **STATUS: OPEN — pending one cold test. NOT declared on judgment (MJ ruling, 2026-08-10).** Everything is built and all copy is MJ-final, but no third party has ever opened the game. **Acceptance criterion, recorded so it doesn't drift:** a friend who has not heard about the game opens the release ZIP with no instructions from MJ beyond the link, and **reaches turn 2 without asking MJ anything.** That closes item 4. Anything they DO have to ask becomes the next help-surface fix.
